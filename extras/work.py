...
if st.sidebar.button("Predict"):
    ...
    if historical_data.empty:
        ...
        # If the selected date is in the past, fetch the exact value
        if num_days == 0:
            historical_value = historical_data[historical_data['Date'] == pd.to_datetime(date_input)]['Close']
            if not historical_value.empty:
                st.write(f"**Actual Price for {stock_name} on {date_input}:** ${historical_value.values[0]:.2f}")
            else:
            ...
            # Apply the multiplying factor for the selected stock
            multiplying_factor = multiplying_factors[stock_name]
            predicted_price_adjusted = predicted_price * multiplying_factor

            # Display the result
            st.write(f"**Predicted Price for {stock_name} on {date_input}:** ${predicted_price_adjusted:.2f}")
